...
- 支付：`PAYMENT_GATING_ENABLED` `PADDLE_ENV` `PADDLE_API_KEY` `PADDLE_WEBHOOK_SECRET` `PADDLE_PRICE_ID_CNY` `PADDLE_PRICE_ID_USD` `PADDLE_SUCCESS_URL` `PADDLE_CANCEL_URL`
- 可观测：`STRUCTURED_LOG` `SENTRY_DSN`
- 备份：`BACKUP_ENABLED` `BACKUP_INTERVAL_MS` `BACKUP_RETENTION_DAYS`

注意：生产环境不得使用默认 `JWT_SECRET`。
若 Paddle 尚未就绪，可先设置 `PAYMENT_GATING_ENABLED=0` 以关闭支付门禁并开放导出；待支付可用后改为 `1`。
